function setConfigurePropertys(){
  ...
  //日本在庫サイトURL列のタイトル名
  properties.setProperty("targetKeyword",configureSheet.getRange('C2').getValue());

  //在庫管理ツールのファイルID
  properties.setProperty("stockManageFileId",configureSheet.getRange('C3').getValue());

  //在庫ワード管理のシート名
  properties.setProperty("wordManageSheetName",configureSheet.getRange('C4').getValue());

  //サイト別監視用WORKファイル一覧のシート名
  properties.setProperty("siteWorkFlieSheetName",configureSheet.getRange('C5').getValue());

  //監視対象ファイル一覧のシート名
  properties.setProperty("targetFileSheetName",configureSheet.getRange('C6').getValue());

  //Workファイル　テンプレートファイルID
  properties.setProperty("templateFileID",configureSheet.getRange('C7').getValue());

  //Workファイル　格納フォルダのID
  properties.setProperty("workFolderId",configureSheet.getRange('C8').getValue());

  //在庫状況表示色　在庫確認待ち（初期状態）
  properties.setProperty("statusColor_wait",configureSheet.getRange('C9').getBackground());

  //在庫状況表示色　在庫あり
  properties.setProperty("statusColor_ok",configureSheet.getRange('C10').getBackground());

  //在庫状況表示色　在庫なし
  properties.setProperty("statusColor_ng",configureSheet.getRange('C11').getBackground());

  //在庫状況表示色　監視処理エラー  
  properties.setProperty("statusColor_err",configureSheet.getRange('C12').getBackground());

  //Phantom APIのAPI-KEY 
  properties.setProperty("PHANTOM_API_KEY",configureSheet.getRange('C13').getValue());

  //ログフォルダのID 
  properties.setProperty("LOG_FOLDER_ID",configureSheet.getRange('C14').getValue());

  configureSheet.getRange('C18').setValue(getDateString(new Date()) + ":初期設定が完了しました。");
...
function monitorTest(){
  ...
  //
  var targetList = testSheet.getRange(3,1,8,10).getValues();
  for (i=0;i<targetList.length;i++) {
    ...
    if (!targetUrl) {
      ...
      var status = getStockStatus(targetUrl,sitekbn,parsePattern,stockCheckWord,parseHtmlFrom,parseHtmlTo);
